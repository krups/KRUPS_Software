...
volatile bool launched = false, ejected = true, splash_down = false, GPS_Mode = false, GPSFail = false, chuteDeployed = false;
...
void do_tasks()
{
  ...
  //check splashDown
  if (millis() > TIME_TO_DEPLOY && !chuteDeployed)
  {
    Serial.println(TIME_TO_DEPLOY);
    Serial.println(millis());
    deployChute();
  }

  //check splashDown
  if (millis() > TIME_TO_SPLASH_DOWN && !splash_down)
  {
...
void setup() {
  ...
  //int powerMode = analogRead(23); //NOT FINAL
  //blinkLed(1, 250); //LED Not visible
...
void loop() {
  ...
  if (splash_down && message_queue.isEmpty()) 
  {
  
  #if USE_MODEM
      int response = isbd.sendSBDText(gpsdata);
  #endif
  ...
  if(chuteDeployed)
  {
    SendGPS(gpsdata); //should attempt GPS after chute
  }
